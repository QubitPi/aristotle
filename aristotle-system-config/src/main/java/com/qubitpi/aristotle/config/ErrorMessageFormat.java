...
public enum ErrorMessageFormat implements MessageFormatter {
     ...
     */
    EMPTY_LIST("Aristotle could not process the request due to an internal error.", "List cannot be empty"),
    ...
    JSON_DESERIALIZATION_ERROR(
            "Aristotle could not process the request because HTTP request body is not properly JSON-formatted: '%s'",
            "Invalid JSON: '%s'"
    ),
    ...
    INVALID_GRAPHQL_REQUEST(
            "Aristotle could not process the request because %s: '%s'",
